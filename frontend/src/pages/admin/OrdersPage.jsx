...
import { Search, Eye, ChevronDown, X, ShoppingCart, Clock, CheckCircle, XCircle, Package } from 'lucide-react'
...
export default function OrdersPage() {
  ...
  const pendingCount = ordersArray.filter(o => o.status === 'pendiente' || o.status === 'pending_payment').length
  const deliveredCount = ordersArray.filter(o => o.status === 'entregado' || o.status === 'delivered').length
  const cancelledCount = ordersArray.filter(o => o.status === 'cancelado' || o.status === 'cancelled').length
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1 text-green-600">{deliveredCount}</p>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold mt-1 text-red-600">{cancelledCount}</p>
            </div>
                ...
                {filteredOrders.map((order, index) => (
                  ...
                  >
                    <td className="px-6 py-4">
                      <span className="text-sm font-bold text-gray-900">#{order.orderNumber || order.id?.slice(0, 8)}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{order.shippingName || order.user?.name}</p>
